...
SCILIB int ClearBuffer(void *buffer_handle)
{
	...
	return 0;
}SCILIB int REG_PERIOD_GET(uint32_t *val, NI_HANDLE *handle)
{
     return __abstracted_reg_read(val, SCI_REG_PERIOD, handle);
}
SCILIB int REG_PERIOD_SET(uint32_t val, NI_HANDLE *handle)
{
     return __abstracted_reg_write(val, SCI_REG_PERIOD, handle);
}
//-----------------------------------------------------------------
//-
...
{
	int r0 = __abstracted_reg_write(length, SCI_REG_Digitizer_0_ACQ_LEN, handle);
	if (r0 == 0)
		return 0;
	else
		return -1;
...
//-     OSCILLOSCOPE_Digitizer_0_DOWNLOAD(data_buffer, BUFFER_SIZE_Digitizer_0, 1000, handle, rd, vp);
//- 
//-
//- ARGUMENTS:
//- 	             val  PARAM_OUT   uint32_t
